...
class Normativo_model extends CI_Model
{
    ...
    public function get_delegacional($filtros = [])
    {
        ...
        $this->db->select(array('del.clave_delegacional', 'del.nombre', 'coalesce(t.total, 0) total'));
    ...
    public function get_umae($filtros = [])
    {
        ...
        $this->db->select(array('u2.unidad_principal', 'u2.nombre_unidad_principal', 'coalesce(sum(t.total), 0) total'));
